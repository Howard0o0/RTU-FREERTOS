...
/******************************************************************************
功能：设置内核电压
详细：Power Management Module (PMM).The PMM manages all functions related to the power supply and
its supervision for the device. Its primary functions are first to generate a supply voltage for the
core logic, and second, provide several mechanisms for the supervision and monitoring of both the
voltage applied to the device (DVCC) and thevoltage generated for the core (VCORE). The PMM uses an
integrated low-dropout voltage regulator (LDO) to produce a secondary core voltage(VCORE) from the
primary one applied to the device (DVCC). In general, VCOREsupplies the CPU, memories(flash and
RAM), and the digital modules, while DVCCsupplies the I/Os and all analog modules (includingthe
oscillators). The VCOREoutput is maintained using a dedicated voltage reference. VCORE is
programmable up to four steps, to provide only as much power as is needed for the speed that has
been selected for the CPU. This enhances power efficiency of the system. The input or primary side
of the regulator is referred to in this chapter as its high side. The output or secondary side is
referred to in this chapter as its low side.
******************************************************************************/
...
char ConvertAscIItoHex(char* ascii, char* hex, int asciilen) {
	...
	//    if((asciilen % 2) == 0)
	//    {
	//        TraceMsg("The size of AscII array is odd, it must be even. The size of hex array
	//        must be even too！",1); return -1;
	//    }
...
{						       //报告时间 5分钟
	...
	char temp[ 2 ];
	int  mode = 0;
	if (Store_ReadReportTimeMode(temp) < 0) {  //没读出模式 就当5分钟一存好了.
		mode = 1;
	}
	else
...
{						     //报告时间 5分钟
	...
	char temp[ 2 ];
	int  mode = 0;
	if (Store_ReadCameraTimeMode(temp) < 0) {  //没读出模式 就当5分钟一存好了.
		mode = 1;
	}
	else
		mode = (temp[ 0 ] - '0') * 10 + temp[ 1 ] - '0';
...
{						     //保存时间 5分钟
	...
	char temp[ 2 ];
	int  mode = 0;
	if (Store_ReadSaveTimeMode(temp) < 0) {  //没读出模式 就当5分钟一存好了.
		mode = 1;
	}
	else
		mode = (temp[ 0 ] - '0') * 10 + temp[ 1 ] - '0';
...
int Utility_Is_A_CheckTime(char* _time) {
	char timerinterval;
	char endtime[ 6 ] = { 0, 0, 0, 0, 0, 0 };
	Hydrology_ReadStoreInfo(HYDROLOGY_SAMPLE_INTERVAL, &timerinterval,
				HYDROLOGY_SAMPLE_INTERVAL_LEN);  // ly
	timerinterval = _BCDtoDEC(timerinterval);
	Utility_Strncpy(endtime, _time, 6);

	if (endtime[ 4 ] % (timerinterval / 60) == 0)
		return 1;
	else
		return 0;
}
...
int Utility_Is_A_SaveTime(char* _time) {
	char timerinterval;
	char endtime[ 6 ] = { 0, 0, 0, 0, 0, 0 };
	Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_STORE_INTERVAL, &timerinterval,
				HYDROLOGY_WATERLEVEL_STORE_INTERVAL_LEN);  // ly
	timerinterval = _BCDtoDEC(timerinterval);
	Utility_Strncpy(endtime, _time, 6);

	if (endtime[ 4 ] % (timerinterval) == 0)
		return 1;
	else
		return 0;
}
...
int Utility_Is_A_CameraTime(char* _time) {
	char _temp[ 2 ];
	int  _mode = 0;
	if (Store_ReadCameraTimeMode(_temp) < 0) {  //没读出模式 就当5分钟一存好了.
		_mode = 0;
	}
	else
		_mode = (_temp[ 0 ] - '0') * 10 + _temp[ 1 ] - '0';
