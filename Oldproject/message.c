...
int Hydrology_ReadDataPacketCount(int *dest)
{
  ...
  char _temp[4];
  Hydrology_ReadStoreInfo(HYDROLOGY_DATA_PACKET_COUNTER ,_temp,HYDROLOGY_DATA_PACKET_COUNTER_LEN);
  (*dest) = (_temp[0]-'0')*1000+(_temp[1]-'0')*100+(_temp[2]-'0')*10+(_temp[3]-'0');
  if((*dest) > HYDROLOGY_DATA_MAX_IDX || (*dest) < HYDROLOGY_DATA_MIN_IDX )
        {
            (*dest) = 0; 
...
int Hydrology_SetDataPacketCount(int src)
{
    ...
    //s_DPCount=src;
    Utility_UintToStr4(src,_dest);
    Hydrology_WriteStoreInfo(HYDROLOGY_DATA_PACKET_COUNTER,_dest,HYDROLOGY_DATA_PACKET_COUNTER_LEN);
    ...
    {//1起头的第2种情况
        ...
        {//遇到0了 就是_startIdx
            ...
            if(_ret==1)
                continue;
            if( _ret==-2)//出边界了
            {//全1,则设 _startIdx = DATA_MIN_IDX   _endIdx = DATA_MIN_IDX
    ...
    {//道理同上 0起头 第一种情况
        ...
        { //找1了 就是endIdx
            ...
            if(_ret==0)
                continue;
            if( _ret==-2)//出边界了
            {//全 0 ,则设 _startIdx = DATA_MIN_IDX   _endIdx = DATA_MIN_IDX
                Hydrology_SetStartIdx(HYDROLOGY_DATA_MIN_IDX);
                Hydrology_SetEndIdx(HYDROLOGY_DATA_MIN_IDX);
...
void Hydrology_ReadAnalog(float *value,int index)   //index表示第index个要素的值
{
...
void Hydrology_CalElementInfo(int *count,char funcode)
{
  ...
  if(type == 1)
  {/*从内存中读取数据inputPara id，num，value*/
    while(Element_table[i].ID != 0)
    {
      switch(Element_table[i].type)
      {
...
int hydrologyMakeDownBody(char* input,int len,int position)
{
  ...
  while(len != 0)
  {
    ...
    if(input[position]==0xFF)       //*********扩展标识符，FFxxxx，后两个字节xx表示是什么要素，xx表示后面跟的data长度，高5位表示字节数，低三位表示小数点后位数****************
    {
          position += 1;
          Extend_Flag = 1;
          len -= 1;
    ...
    (pbody->element)[pbody->count].value = (char*) malloc((pbody->element)[pbody->count].num);                                           //根据num开辟data空间
...
int hydrologyProcessReceieve(char* input,int inputlen)
{
  ...
  if(hydrologyMakeDownHeader(input,inputlen,&i,&bodylen) != 0)     //i是postion  ；bodylen，报文正文长度
    return -1;
