...
int GSM_AT_QueryTime(char* year, char* month, char* date, char* hour, char* min, char* second) {
	...
	char*	pcAtResp      = NULL;
	char*	pcSeperate    = "/";
	static char* pcRcvAtBuff = NULL;  //���徲̬�ֲ�������������ȫ��������
	...
	while (_repeat < 10) {
		if ((UART0_RecvLineTry(pcRcvAtBuff, 200, &_retNum) == 0)
		    && ((pcAtResp = strstr(pcRcvAtBuff, pcSeperate)) != NULL))  //������
		{
			rcvFlag = 1;
			break;
		}
...
int GSM_Open() {
	...
	//��ʼ������0,
	UART0_Open(
		UART0_GTM900_TYPE);  // lmj0814,GTM900
				     // ATָ�������Ҫ���ӵ���\r\n,��mc35i������ͬ����ˣ��ڴ��ڷ��ͺ������棬��󸽼�\r\n
	...
	P3OUT &= ~BIT0;  // ��/IGTһ���½�,����100ms
			 //    System_Delayms(50);  // ��Ҫ��ʱ

	//    P3DIR &= ~BIT3;
	System_Delayms(50);  // GTM900��Ҫ50ms
	// System_Delayms(50); // MC35i��Ҫ150ms
	P3OUT |= BIT0;  // �ٰ�/IGT ����
...
int GSM_Report_Process() {  //����������.
	...
	_data[ 12 ] = '>';
	if (_endIdx < DATA_MIN_IDX || _endIdx > DATA_MAX_IDX || _startIdx < DATA_MIN_IDX
	    || _startIdx > DATA_MAX_IDX) {
		return -1;
	}
	//
	//   1. ��װ ����
	...
	while (1) {  //����ѭ��
		...
		while (1) {  //�����ܾ��Ա�����ѭ����?
			...
			if (_ret
			    == 1) {  //�����һ���Ѿ����͹�������,��ô�Ͳ�����
				if (_startIdx == _endIdx) {  //����Ƿ���  _endIdx, ����ǾͲ�����ѭ����.
					_exit = 1;
					//�����װ.
					_data[ _idx - 1 ] = '#';  //�����һ��;�Ÿ�Ϊ#
					break;
				...
				//
				if (_seek_num
				    > DATA_MAX_IDX - DATA_MIN_IDX) {  //�����ж�һ��,���������ѭ��.
					_exit		  = 1;
					_data[ _idx - 1 ] = '#';  //�����һ��;�Ÿ�Ϊ#
					break;
				}
		...
		{
			for (int i = 1; i <= 4; ++i) {
				if (Store_GSM_ReadCenterPhone(i, _phone) < 0)
					continue;
			...
			if (_send_ret
			    == 0)  //�������а�������������Ŀ�����óɷ��͹�.
			{
				for (int i = 0; i < _array_idx; ++i) {  //����Ϊ�Ѿ�����
...
int GSM_CheckLive() {  //����AT����Ƿ�����Ӧ,����Ӧ����ΪGSMģ�黵����
	int _repeats1 = 0;
	// int _repeats2=0;
...
int GSM_Init()  // SIM��ʼ����
{
	...
	// int _repeats3=0;
	s_NetState = '0';
	while (GSM_AT_CloseFeedback() != 0) {  //�رջ���,һ��Ҫ�ɹ�, ��Ȼ�������붼�Ǵ����.
		if (_repeats > 2) {  // 3�ζ��Ǵ����.
	...
	while (GSM_AT_QuerySim() != 0) {  //��ѯSIM���Ƿ�����.
		if (_repeats > 2) {       // 3�ζ��Ǵ����.
...
ProcMessage:
	...
	*/
	...
	System_Delayms(50);
	if (UART0_RecvLineWait(_recv, UART0_MAXBUFFLEN, &_retNum) == -1) {  //δ���յ�����ͷ
	...
	if (Utility_Strncmp(_recv, "+CMGR: ", 7) == 0) {  //��õ��˶���ͷ,�������ж������λ���ǲ���û�ж���
							  //�޶���. "+CMGR: 0,,0"
		...
		if (Utility_Strncmp(&(_recv[ 8 ]), "REC", 3) != 0) {
			System_Delayms(1000);
			//�ȰѺ����Ķ�����������.
			if (UART0_RecvLineLongWait(_recv, UART0_MAXBUFFLEN, &_retNum)
			    < 0)  //���ȴ��ɴ�5��
			{  //�ղ���Ҳ����ν.�������ǲ���Ҫ.
				;
			}
...
int GSM_ProcMsgArray() {  //ֱ�������±� �ﵽ������������
	int _ret;
	while (s_ProcIdx != s_RecvIdx) {  //������������ķ���ֵ ���� ���ڲ����õķ��Ͷ��ź����ķ���ֵ
					  //�޶��� �᷵�� 1
...
SendMsgTxt:
	...
	*/
	...
	if (UART0_RecvLineWait(_send, 60, &_retNum) == -1) {  //���ŷ��� δ�ɹ�
		++s_MsgFail;
		GSM_AT_QuitInput();
		if (_repeats1 > 2) {
	...
	if (Utility_Strncmp(_send, "+CMGS: ", 7) == 0) {  //�ɹ�
...
int GSM_ProcMsgData(char* _data, char* _rePhone, int _dataLen) {
	...
	_send[ 12 ] = '<';
	//���*��
	if (_data[ 5 ] != '*' || _data[ 8 ] != '*') {  //����Ͳ�������.��ֹ��Ϊ�Լ����Լ���,������ѭ��.
	...
	case 'B':
		...
		if (_data[ 7 ] != 'S' || _dataLen != 32 || _data[ 20 ] != ',' || _data[ 25 ] != ','
		    || _data[ 28 ] != ',' || Utility_CheckDigital(_data, 9, 19)
		    || Utility_CheckDigital(_data, 21, 24) || Utility_CheckDigital(_data, 26, 27)
		    || Utility_CheckDigital(_data, 29, 30)) {
			_ret = Utility_PackErrorMsg("UN", _send);
			return GSM_SendMsgTxt(_rePhone, _send, _ret);
		}
		TraceMsg("BS msg ", 1);
		...
		if (_data[ 21 ] != '0' || _data[ 21 ] != '0' || _data[ 21 ] != '0'
		    || _data[ 21 ] != '0') {
	...
	case 'C':
		...
		//$1234*CS*P,11000000#
		if (_data[ 7 ] == 'P' && _data[ 10 ] == ','
		    && (!Utility_CheckDigital(_data, 11, 21))) {
			TraceMsg("CP msg ", 1);
		...
		if (_data[ 7 ] == 'S' && _data[ 10 ] == ','
		    && (!Utility_CheckBinary(_data, 11, 18))) {
			TraceMsg("CS msg ", 1);
			_tempChar1 = 0x00;
			...
			if (_data[ 9 ] == 'A') {		 //ģ���� ,��λ��ǰ
				for (int i = 11; i < 19; ++i) {  //$1234*CS*P,11000000#
					_tempChar1 >>= 1;
					if (_data[ i ] != '0')
						_tempChar1 |= BIT7;
					else
	...
	case 'D':
		...
		//$1234*DL*A,0000,4096#
		if (_data[ 7 ] != 'L' || _data[ 10 ] != ',' || _data[ 15 ] != ','
		    || Utility_CheckDigital(_data, 11, 14) || Utility_CheckDigital(_data, 16, 19)) {
			_ret = Utility_PackErrorMsg("UN", _send);
			return GSM_SendMsgTxt(_rePhone, _send, _ret);
		}
		TraceMsg("DL msg ", 1);
		...
		_max  = 0;
		_idx1 = _data[ 9 ] - 'A' + 1;
		if (_data[ 11 ] != '9' || _data[ 12 ] != '9' || _data[ 13 ] != '9'
		    || _data[ 14 ] != '9') {
		...
		if (_data[ 16 ] != '9' || _data[ 17 ] != '9' || _data[ 18 ] != '9'
		    || _data[ 19 ] != '9') {
	...
	case 'P':
		...
		//$1234*PM*K,6#
		//$1234*PV*L,1234567#
		if ((_data[ 7 ] != 'L' && _data[ 7 ] != 'R' && _data[ 7 ] != 'M'
		     && _data[ 7 ] != 'V' && _data[ 7 ] != 'D')
		    || _data[ 10 ] != ',') {
			_ret = Utility_PackErrorMsg("UN", _send);
			return GSM_SendMsgTxt(_rePhone, _send, _ret);
		...
		if (_data[ 7 ] == 'L') {
			TraceMsg("PL msg ", 1);
			if (_data[ 9 ] < 'M' || _data[ 9 ] > 'T'
			    || (_data[ 11 ] != '1' && _data[ 11 ] != '0')) {
		...
		//$1234*PR*I,100#
		if (_data[ 7 ] == 'R') {
			if (_data[ 9 ] < 'I' || _data[ 9 ] > 'L'
			    || Utility_CheckDigital(_data, 11, 13)) {
		...
		//$1234*PM*I,6#
		if (_data[ 7 ] == 'M') {
			if (_data[ 9 ] < 'I' || _data[ 9 ] > 'L' || _data[ 11 ] < '1'
			    || _data[ 11 ] > '7') {
				_ret = Utility_PackBadMsg("PR", _send);
				return GSM_SendMsgTxt(_rePhone, _send, _ret);
			}
			...
			if (_ret < 0) {
				...
				//���޸��ڴ�ֵ
				_ret += Store_ReadPulseRangeBytes(_idx1,
								  g_pulse_range[ _idx1 - 1 ]);
		...
		if (_data[ 7 ] == 'V') {
			if (_data[ 9 ] < 'I' || _data[ 9 ] > 'L'
			    || Utility_CheckDigital(_data, 11, 17)) {
	...
	case 'W':
		...
		if (_ret < 0) {
			...
			//�������� û��ɾ�� ��ֱ��������. ���Իᱻ�ظ�2��.�����ʱ���ø�.
			if (g_work_mode != (_data[ 9 ] - '0')) {  //���ģʽ��һ��, ��ô��������.
				// �������ֱ������ ����� WM���ŵ���һֱ����������.
				//���Ըĳ�ģʽ��һ��������.
				GSM_Close(0);  //�ر�GSM
				System_Delayms(1000);
	...
	case 'R':
		if (_data[ 7 ] == 'P' && _data[ 8 ] == '*') {
			if (_data[ 9 ] == 'B'
			    && _data[ 10 ] == 'S') {  // 012345678901234567890123456789
			...
			if (_data[ 9 ] == 'D'
			    && _data[ 10 ] == 'L') {  // 0123456789012345678901234567890123
			...
			if (_data[ 9 ] == 'C'
			    && _data[ 10 ] == 'P') {  // 012345678901234567890123456789
			...
			if (_data[ 9 ] == 'T'
			    && _data[ 10 ] == 'M') {  // 01234567890123456789012345678
			...
			if (_data[ 9 ] == 'P' && _data[ 10 ] == 'L') {  // 0123456789012345678901234
			...
			if (_data[ 9 ] == 'C' && _data[ 10 ] == 'S') {  //          1         2 3 4
			...
			if (_data[ 9 ] == 'P'
			    && _data[ 10 ] == 'R') {  // 012345678901234567890123456789012
			...
			if (_data[ 9 ] == 'P' && _data[ 10 ] == 'M') {  // 0123456789012345678901234
			...
			if (_data[ 9 ] == 'W' && _data[ 10 ] == 'M') {  // 012345678901234567890
				//$00011100011<WM*0#
			...
			if (_data[ 9 ] == 'P' && _data[ 10 ] == 'V') {  // 0         1         2 3 4
...
int GSM_GetRePhone(char* src,
		   char* dest) {  // +CMGR: "REC UNREAD","+8613607193119",,"09/02/27,16:36:16+32"��ʽ
	int index = 0;
	while (src[ index++ ] != ',')
		;		    //��һ��,��֮����ǻظ��绰
	++index;		    // indexָ��"��
	if (src[ index ] == '+') {  //��+86,��Ҫ������3���ַ�
		index += 3;
	}
	else  //û��+,GTM900��ʽ��
	{
...
int GSM_CallLeader(char* _phone) {
	...
	for (int i = 0; i < 5; ++i) {
		...
		{
			if (Utility_Strncmp(_recv, "OK", 2) == 0) {  //�����˽���绰��. Ӧ�ùҶ�
				GSM_AT_OffCall();
			}
			//������������ڴ�绰 ���� No Carrier
			//��������˹Ҷ�,  ģ�龹Ȼ û�з�Ӧ.. ����..������.�������15������.
			//����û�źŵĻ�, ����Ҳ���������...
			if (Utility_Strncmp(_recv, "NO CARRIER", 10)
			    == 0) {  //���ǾͲ�����.������,�������������ǰ,�Ѿ��ж��ŷ��ͳɹ�.
				     //���Բ�̫������,SIMģ��û׼����
				     //�� NO CARRIER ������������ �Ƚϰ�ȫ��
				GSM_AT_OffCall();
...
int GSM_DealData(char* _recv, int _dataLen) {
	...
	//  ����������
	//
	if (Utility_Strncmp(_recv, "RING", 4) == 0) {  //�绰
		++s_RING;
		TraceMsg(" RING !", 1);
...
int GSM_AT_QueryNet() {
	...
	if (Utility_Strncmp(_recv, "+CREG: ", 7) == 0) {  //�������״̬
		s_NetState = _recv[ 9 ];
		if (UART0_RecvLineWait(_recv, 30, &_retNum) == -1) {
			++s_TimeOut;
			return -1;
...
SHUTDOWN:
	...
	if (Utility_Strncmp(_recv, "^SMSO: MS OFF", 13) == 0) {  //�ٽӸ�OK
