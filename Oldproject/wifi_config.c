...
//#endif
    ...
    pvPortMallocsize = (recvLength - A_STEP_LEN - C_STEP_LEN - 4) * sizeof(char);
    TraceMsg("pvPortMalloc size:",1);
    TraceInt4(pvPortMallocsize,1);
    
    char *hydrologyPara = (char *) pvPortMalloc(pvPortMallocsize);//6�ǰ�ͷ�����ݳ�����ռ�ֽ�,1���ֽڽ���������ȥ7���ֽ�
    if(hydrologyPara == 0)
    {
        Console_WriteErrorStringln("0 memory pvPortMalloced");
        return -1;
    }
    
    Utility_Strncpy(hydrologyPara, &dest[3 + A_STEP_LEN],pvPortMallocsize);

    _ret = Main_ProcInitConfig(hydrologyPara, pvPortMallocsize, &_step);
    
    vPortFree(hydrologyPara);
