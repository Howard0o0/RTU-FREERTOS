...
void RS485_ValueDefine(char* buffer, char* value, int index, int times) {
	...
	Hydrology_ReadStoreInfo(HYDROLOGY_RS485_RECV_DATA_ORDER1
					+ HYDROLOGY_RS485_RECV_DATA_ORDER_LEN * index,
				&data_order, HYDROLOGY_RS485_RECV_DATA_ORDER_LEN);

	//+++++
	switch (data_order)  //+++++++++++++4.��������˳��++++�ֽ�˳��+++++++++
	{
	...
	case 5: {
		intvalue = ( uint32_t )buffer[ 3 ] * 16777216 + ( uint32_t )buffer[ 2 ] * 65536
	...
	case 6: {
		intvalue = ( uint32_t )buffer[ 2 ] * 16777216 + ( uint32_t )buffer[ 3 ] * 65536
	...
	case 7: {
		intvalue = ( uint32_t )buffer[ 1 ] * 16777216 + ( uint32_t )buffer[ 0 ] * 65536
	...
	case 8: {
		intvalue = ( uint32_t )buffer[ 0 ] * 16777216 + ( uint32_t )buffer[ 1 ] * 65536
	...
	case 9: {
		floatvalue = buffer[ 3 ] * 16777216 + buffer[ 2 ] * 65536 + buffer[ 1 ] * 256
			     + buffer[ 0 ];
		floatvalue /= 1000;
...
void RS485_Send(char device, char function, char reg_addrH, char reg_addrL, char timeout,
	...
	char crc_type	     = 0;
	char _crc_order[ 2 ] = { 0 };
	Hydrology_ReadStoreInfo(HYDROLOGY_RS485_CRC_TYPE1 + HYDROLOGY_RS485_CRC_TYPE_LEN * index,
				&crc_type, HYDROLOGY_RS485_CRC_TYPE_LEN);
	Hydrology_ReadStoreInfo(HYDROLOGY_RS485_CRC_ORDER1 + HYDROLOGY_RS485_CRC_ORDER_LEN * index,
	...
	command[ 5 ] = 2;
	if (crc_type == 0x01) {
		crc = hydrologyCRC16(command,
				     6);  //++++++++++++1.����crcУ�����ͺ�2.��С��˳��++++++++++++++

		if (_crc_order[ 0 ] == 0x01)  //��λ��ǰ����λ�ں�
		{
			command[ 6 ] = crc >> 8;
			command[ 7 ] = crc % 256;
		}
...
int RS485_Read(char* buffer, int index) {
	...
	if (recv_crc_order == 0x01)  ////��λ��ǰ����λ�ں�
	{
		crc1 = hydrologyCRC16(temp_buffer,
				      7);  //++++++++++++5.����crcУ���С��˳��++++++++++++++
		crc2 = temp_buffer[ 7 ] << 8 | temp_buffer[ 8 ];
	}
	else if (recv_crc_order == 0x02)  //��λ��ǰ����λ�ں�
	{
...
void Hydrology_ReadRS485(char* value, int index) {
	...
	while (single_count1--) {
		...
		Hydrology_ReadStoreInfo(HYDROLOGY_RS485_RECV_DELAY1
						+ HYDROLOGY_RS485_RECV_DELAY_LEN * index,
		...
		for (error = 0; error < 3; error++) {
			...
			RS485_Send(temp_value[ 0 ], temp_value[ 1 ], temp_value[ 2 ],
				   temp_value[ 3 ], temp_value[ 4 ], index);
			RS485_Delayms(_delay_time);  //++++++++++++3.������ʱ++++++++5000+++++
...
//1�Ƕ�P2�������룬2��io9-16��3��io17-24����P8�������루��ʱ���ã�++++++++++++++++++++++++++++++++++
{
   //������
...
void Hydrology_ReadIO_STATUS(
	char* value,
	int   flag)  // 1�Ƕ�P2�������룬2��io9-16��3��io17-24����P8�������루��ʱ���ã�++++++++++++++++++++++++++++++++++
{
	//������
...
int hydrologyConfigSelect(char* guide, char* value) {
	...
	case 0x0E: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_REPEATER_STATION, value,
...
int hydrologySetParaSelect(char* guide, char* value) {
	...
	case 0x22: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_RAINFALL_BEGIN_TIME, value,
	...
	case 0x24: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_STORE_INTERVAL, value,
					       HYDROLOGY_WATERLEVEL_STORE_INTERVAL_LEN);
		break;
	}
	case 0x25: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_RAINFALL_RESOLUTION, value,
					       HYDROLOGY_RAINFALL_RESOLUTION_LEN);
		break;
	}
	case 0x26: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_RESOLUTION, value,
					       HYDROLOGY_WATERLEVEL_RESOLUTION_LEN);
		break;
	}
	case 0x27: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_RAINFALL_ADD_THRESHOLD, value,
					       HYDROLOGY_RAINFALL_ADD_THRESHOLD_LEN);
		break;
	}
	case 0x28: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE1, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE1_LEN);
		break;
	}
	case 0x29: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE2, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE2_LEN);
		break;
	}
	case 0x2A: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE3, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE3_LEN);
		break;
	}
	case 0x2B: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE4, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE4_LEN);
		break;
	}
	case 0x2C: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE5, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE5_LEN);
		break;
	}
	case 0x2D: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE6, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE6_LEN);
		break;
	}
	case 0x2E: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE7, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE7_LEN);
		break;
	}
	case 0x2F: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE8, value,
					       HYDROLOGY_WATERLEVEL_BASICVALUE8_LEN);
		break;
	}
	case 0x30: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE1, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE1_LEN);
		break;
	}
	case 0x31: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE2, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE2_LEN);
		break;
	}
	case 0x32: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE3, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE3_LEN);
		break;
	}
	case 0x33: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE4, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE4_LEN);
		break;
	}
	case 0x34: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE5, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE5_LEN);
		break;
	}
	case 0x35: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE6, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE6_LEN);
		break;
	}
	case 0x36: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE7, value,
					       HYDROLOGY_WATERLEVEL_CORRECTVALUE7_LEN);
		break;
	}
	case 0x37: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE8, value,
	...
	case 0x40: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_ADD_ABOVE_THRESHOLD, value,
					       HYDROLOGY_ADD_ABOVE_THRESHOLD_LEN);
		break;
	}
	case 0x41: {
		ret = Hydrology_WriteStoreInfo(HYDROLOGY_ADD_BELOW_THRESHOLD, value,
	...
	case 0xFF: {
		// ret = Hydrology_WriteElementTable();  //����0xff������һ��element
		// table���Ԫ��
		break;
	}
	default: {
...
int hydrologyReadParaSelect(char* guide, char* value) {
	...
	case 0x22: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RAINFALL_BEGIN_TIME, value,
					      HYDROLOGY_RAINFALL_BEGIN_TIME_LEN);
		break;
	}
	case 0x23: {
	...
	case 0x24: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_STORE_INTERVAL, value,
					      HYDROLOGY_WATERLEVEL_STORE_INTERVAL_LEN);
		break;
	}
	case 0x25: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RAINFALL_RESOLUTION, value,
					      HYDROLOGY_RAINFALL_RESOLUTION_LEN);
		break;
	}
	case 0x26: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_RESOLUTION, value,
					      HYDROLOGY_WATERLEVEL_RESOLUTION_LEN);
		break;
	}
	case 0x27: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RAINFALL_ADD_THRESHOLD, value,
					      HYDROLOGY_RAINFALL_ADD_THRESHOLD_LEN);
		break;
	}
	case 0x28: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE1, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE1_LEN);
		break;
	}
	case 0x29: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE2, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE2_LEN);
		break;
	}
	case 0x2A: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE3, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE3_LEN);
		break;
	}
	case 0x2B: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE4, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE4_LEN);
		break;
	}
	case 0x2C: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE5, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE5_LEN);
		break;
	}
	case 0x2D: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE6, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE6_LEN);
		break;
	}
	case 0x2E: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE7, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE7_LEN);
		break;
	}
	case 0x2F: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_BASICVALUE8, value,
					      HYDROLOGY_WATERLEVEL_BASICVALUE8_LEN);
		break;
	}
	case 0x30: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE1, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE1_LEN);
		break;
	}
	case 0x31: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE2, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE2_LEN);
		break;
	}
	case 0x32: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE3, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE3_LEN);
		break;
	}
	case 0x33: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE4, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE4_LEN);
		break;
	}
	case 0x34: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE5, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE5_LEN);
		break;
	}
	case 0x35: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE6, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE6_LEN);
		break;
	}
	case 0x36: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE7, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE7_LEN);
		break;
	}
	case 0x37: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_WATERLEVEL_CORRECTVALUE8, value,
					      HYDROLOGY_WATERLEVEL_CORRECTVALUE8_LEN);
		break;
	}
	case 0x38: {
	...
	case 0x40: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_ADD_ABOVE_THRESHOLD, value,
					      HYDROLOGY_ADD_ABOVE_THRESHOLD_LEN);
		break;
	}
	case 0x41: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_ADD_BELOW_THRESHOLD, value,
					      HYDROLOGY_ADD_BELOW_THRESHOLD_LEN);
		break;
	}
	case 0xA9: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT1, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xAA: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT2, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xAB: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT3, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xAC: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT4, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xAD: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT5, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xAE: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT6, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xAF: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT7, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xB0: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT8, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xB1: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT9, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xB2: {
		ret = Hydrology_ReadStoreInfo(HYDROLOGY_RS485_COUNT10, value,
					      HYDROLOGY_RS485_COUNT_LEN);
		break;
	}
	case 0xB3: {
...
int hydrologySetPara(void) {
	    ...
	    == 1)  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	{
		...
		for(i=0;i<count;i++)
		{
		    if((pbody->element)[i].guide[0]==0xF3)  //ͼƬ��
		 {
		    //����ͼƬ��flag��1����flag�ͷ���ͼƬ�ļ��
		...
		for (i = 0; i < UserElementCount; i++) {
			...
			Hydrology_WriteStoreInfo(
				HYDROLOGY_ELEMENT1_ID + i * HYDROLOGY_ELEMENT_ID_LEN,
				&(pbody->element)[ i ].guide[ 0 ], HYDROLOGY_ELEMENT_ID_LEN);
			Hydrology_WriteStoreInfo(
				HYDROLOGY_ELEMENT1_TYPE + i * HYDROLOGY_ELEMENT_TYPE_LEN,
				&(pbody->element)[ i ].value[ 0 ], HYDROLOGY_ELEMENT_TYPE_LEN);
			Hydrology_WriteStoreInfo(
				HYDROLOGY_ELEMENT1_MODE + i * HYDROLOGY_ELEMENT_MODE_LEN,
				&(pbody->element)[ i ].value[ 1 ], HYDROLOGY_ELEMENT_MODE_LEN);
			Hydrology_WriteStoreInfo(
				HYDROLOGY_ELEMENT1_CHANNEL + i * HYDROLOGY_ELEMENT_CHANNEL_LEN,
				&(pbody->element)[ i ].value[ 2 ], HYDROLOGY_ELEMENT_CHANNEL_LEN);
			/*����ID��ȡD,d*/
			getElementDd(Element_table[ i ].ID, &Element_table[ i ].D,
				     &Element_table[ i ].d);
			if ((pbody->element)[ i ].value[ 1 ] == RS485) {
				temp[ 0 ] = (pbody->element)[ i ].value[ 3 ];  //�Ĵ�������
				Hydrology_WriteStoreInfo(HYDROLOGY_RS485_COUNT1
								 + j * HYDROLOGY_RS485_COUNT_LEN,
							 temp, HYDROLOGY_RS485_COUNT_LEN);
				memcpy(temp, &(pbody->element)[ i ].value[ 5 ],
				       (pbody->element)[ i ]
					       .value[ 4 ]);  //(pbody->element)[i].value[4]
							      //=HYDROLOGY_RS485_LEN   485����
				Hydrology_WriteStoreInfo(HYDROLOGY_RS4851 + j * HYDROLOGY_RS485_LEN,
							 temp, HYDROLOGY_RS485_LEN);

				//+++++++++++++++++++++++++++++��չ����++++++++++++++++++++++++++++++++++++++++++++
				int offt  = 10;
				temp[ 0 ] = (pbody->element)[ i ].value[ offt ];
				Hydrology_WriteStoreInfo(HYDROLOGY_RS485_CRC_TYPE1
				...
				Hydrology_WriteStoreInfo(
					...
					temp, HYDROLOGY_RS485_RECV_DATA_ORDER_LEN);
				offt += 1;
				temp[ 0 ] = (pbody->element)[ i ].value[ offt ];
				Hydrology_WriteStoreInfo(
					HYDROLOGY_RS485_RECV_CRC_ORDER1
						+ j * HYDROLOGY_RS485_RECV_CRC_ORDER_LEN,
					temp, HYDROLOGY_RS485_RECV_CRC_ORDER_LEN);
				offt += 1;
				//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				j++;  // 485Ҫ�ص�����
			}
			if ((pbody->element)[ i ].value[ 1 ]
			    == ISR_COUNT)  //���������ж��ǵ�·�����Ļ������ϼ����ģ����������ϼ�����flag=1
			{
				if ((pbody->element)[ i ].guide[ 0 ] == 0x20
				    || (pbody->element)[ i ].guide[ 0 ] == 0x26) {
					isr_count_flag = 1;
					Hydrology_WriteStoreInfo(
						HYDROLOGY_ISR_COUNT_FLAG, &isr_count_flag,
						HYDROLOGY_ISR_COUNT_FLAG_LEN);	//��ʼ������ڴ���0
				}
				else {
					Hydrology_WriteStoreInfo(HYDROLOGY_ISR_COUNT_FLAG,
								 &isr_count_flag,
			...
			if ((pbody->element)[ i ].value[ 1 ] == IO_STATUS) {
				...
				Hydrology_WriteStoreInfo(HYDROLOGY_SWITCH_PARSE_TYPE,
							 &switch_parse_type,
							 HYDROLOGY_SWITCH_PARSE_TYPE_LEN);
			...
			if ((pbody->element)[ i ].value[ 1 ] == ADC) {
				...
				memcpy(temp, &(pbody->element)[ i ].value[ offset ],
				       HYDROLOGY_ADC_RANGE_LEN);
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_RANGE1
				...
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_OFFSET1
				...
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_BASE1
				...
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_UPPER1
				...
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_LOWER1
				...
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_WARNNING1_1
				...
				memcpy(temp, &(pbody->element)[ i ].value[ offset ],
				       HYDROLOGY_ADC_WARNNING_LEN);
				Hydrology_WriteStoreInfo(HYDROLOGY_ADC_WARNNING2_1
								 + adc * HYDROLOGY_ADC_WARNNING_LEN,
							 temp, HYDROLOGY_ADC_WARNNING_LEN);
				offset += HYDROLOGY_ADC_WARNNING_LEN;
		...
		Hydrology_WriteStoreInfo(HYDROLOGY_ELEMENT1_ID + i * HYDROLOGY_ELEMENT_ID_LEN, &end,
					 HYDROLOGY_ELEMENT_ID_LEN);
		Hydrology_WriteStoreInfo(HYDROLOGY_ELEMENT1_TYPE + i * HYDROLOGY_ELEMENT_TYPE_LEN,
					 &end, HYDROLOGY_ELEMENT_TYPE_LEN);
		Hydrology_WriteStoreInfo(HYDROLOGY_ELEMENT1_MODE + i * HYDROLOGY_ELEMENT_MODE_LEN,
					 &end, HYDROLOGY_ELEMENT_MODE_LEN);
		Hydrology_WriteStoreInfo(HYDROLOGY_ELEMENT1_CHANNEL
		...
		for (i = 0; i < pbody->count; i++) {
			ret = hydrologySetParaSelect((pbody->element)[ i ].guide,
						     (pbody->element)[ i ].value);
			if (ret == -1)
				return i + 1;
		}
...
int Hydrology_Reset(void) {
	...
	temp[ 5 ] = 0x00;
	for (i = 0; i < 8; i++) {
		Hydrology_WriteStoreInfo(HYDROLOGY_ISR_COUNT1 + i * HYDROLOGY_ISR_COUNT_LEN, temp,
					 HYDROLOGY_ISR_COUNT_LEN);
	}
	temp[ 0 ] = 0x00;
	Hydrology_WriteStoreInfo(HYDROLOGY_ISR_COUNT_FLAG, temp,
				 HYDROLOGY_ISR_COUNT_FLAG_LEN);	 //���������ʽ��0Ϊ��·��1Ϊ����

	Hydrology_SetStartIdx(1);  // 0,0�ɲ���retrive����
	Hydrology_SetEndIdx(1);
	Hydrology_SetDataPacketCount(0);  //��Ч���ݰ���1
	// Store_ClearWork();  //����ȫ�����Ϊ�ѷ���
	//  temp[0] = 1;
...
int Hydrology_SetPassword(void) {
	...
	Hydrology_WriteStoreInfo(HYDROLOGY_PASSWORD_ADDR, (pbody->element)[ 0 ].value,
				 HYDROLOGY_PASSWORD_LEN);
...
int hydrologySetICCard(void) {
	...
	Hydrology_WriteStoreInfo(HYDROLOGY_STATUS_ALARM_INFO, (pbody->element)[ 0 ].value,
...
int hydrologyWaterSetting(void) {
	...
	Hydrology_WriteStoreInfo(HYDROLOGY_WATERSETTING, (pbody->element)[ 0 ].value,
