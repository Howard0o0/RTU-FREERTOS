...
void HydrologyUpdateElementTable() {
    ...
    for (i = 0; i < UserElementCount; i++) {
        ...
        Hydrology_ReadStoreInfo(HYDROLOGY_ELEMENT1_ID +
...
int HydrologySample(char *_saveTime) {
    ...
    while (Element_table[i].ID != 0) {
        ...
        case ANALOG: {
            ...
            Hydrology_SetObservationTime(Element_table[i].ID, _temp_sampertime,
                                         i);
            Hydrology_WriteStoreInfo(HYDROLOGY_ANALOG1 +
                                         a * HYDROLOGY_ANALOG_LEN,
        ...
        case PULSE: {
            ...
            Hydrology_SetObservationTime(Element_table[i].ID, _temp_sampertime,
                                         i);
            Hydrology_WriteStoreInfo(HYDROLOGY_PULSE1 + b * HYDROLOGY_PULSE_LEN,
        ...
        case SWITCH: {
            ...
            Hydrology_SetObservationTime(Element_table[i].ID, _temp_sampertime,
                                         i);
            Hydrology_WriteStoreInfo(HYDROLOGY_SWITCH1 +
                                         c * HYDROLOGY_SWITCH_LEN,
...
int HydrologyInstantWaterLevel(char *_saveTime)   //??˙Ä??????ßÿ????°¿??˜l?????
{

    static char endtime[6] = {0, 0, 0, 0, 0, 0};
    Utility_Strncpy(endtime, _saveTime, 6);
    int ret = 0;
    ret     = Utility_Is_A_ReportTime(endtime);   //?????ßÿ??????????
...
*/
    int _effect_count = 0;   //?õ•??flash????ßπ¶ƒ??????????
    Hydrology_ReadDataPacketCount(&_effect_count);   //???????????¶ƒ?????????????
...
int Hydrology_TimeCheck() {
    ...
    if (RTC_IsBadTime(g_rtc_nowTime, 1) <
        0) {   //????????????????,?????????????
