...
{ 
  ...
  len = camera_DatapackRead(device);//获取 pic_packet ，pic_bytes 值
  if (len <0) return -1;                                                 
...
int camera_Take(char device)
{
  ...
  while (camera->count < camera->pack_num_rc) //当还有包没有获取时循环
  { 
    camera->count++;
...
char camera_change(char device)
{
  switch (device)
  {
    case 0: 
    {
      P10OUT |= BIT2;//p102高就选通J14 1 3 5引脚
      break;
    }
    case 1: 
    {
      P10OUT &=~ BIT2;//p102低就选通J14 7 9 11引脚
      break;
    }
    default: 
    {
